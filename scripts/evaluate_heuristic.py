...
def remove_unnecessary_token(text: str):
...
def concat_instruction_input_for_heuristics(examples):
    instruction = remove_unnecessary_token(examples['instruction'])
    if examples["input"]:
        inputs = remove_unnecessary_token(examples['input'])
        source = FIXED_TOKENS["instruction"] + instruction + FIXED_TOKENS["input"] + inputs + FIXED_TOKENS["output"]
    else:
        source = FIXED_TOKENS["instruction"] + instruction + FIXED_TOKENS["output"]
    return {"text": source}
...
def eval_heuristic(args):
    ...
    directory = f"{results_folder}/{eval_type}/heuristic"
    ...
    for split in ["validation_seen", "validation_unseen", "validation_human"]:
        ...
        dataset_split = dataset[split].map(concat_instruction_input_for_heuristics, batched=False, num_proc=8)
        results = {"prompt": [],
                   "tokens" : [],
                   "gen_texts": [],
                   "token_counts": [],
        ...
        df.to_csv(f"{directory}/{args.gen_model}_{split}_heuristic.csv", index=False)

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--gen_model", type=str, default="gpt2-xl")
